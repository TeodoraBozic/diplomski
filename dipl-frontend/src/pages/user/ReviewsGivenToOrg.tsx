...
import type { ReviewUserToOrgIn } from "../../types/api";
import { showToast } from "../../components/Toast";
import { StarIcon } from "@heroicons/react/24/solid";
import { StarIcon as StarOutlineIcon, ArrowLeftIcon, SparklesIcon } from "@heroicons/react/24/outline";

export function ReviewsGivenToOrg() {
  const { eventId } = useParams<{ eventId: string }>();
  const navigate = useNavigate();
  const [formData, setFormData] = useState<ReviewUserToOrgIn>({
    rating: 5,
    comment: "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!eventId) {
      setError("ID događaja je obavezan");
      return;
    }
    ...
    try {
      await reviewsApi.createUserToOrgReview(eventId, formData);
      ...
      // Proveri da li je greška vezana za završetak događaja ili 500 greška
      const errorLower = errorMessage.toLowerCase();
      if (err?.status === 500 || 
          errorLower.includes("završen") || 
          errorLower.includes("ended") ||
          errorLower.includes("finished") ||
          errorLower.includes("not finished") ||
          errorLower.includes("event") && errorLower.includes("not")) {
        errorMessage = "Događaj još nije završen ili je došlo do greške. Ocena se može dati samo nakon završetka događaja.";
        // Navigiraj nazad nakon 3 sekunde
        setTimeout(() => {
  ...
  return (
    <div className="max-w-3xl mx-auto animate-fade-in-up">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-4xl font-bold text-[#121212]">Oceni organizaciju</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card p-8 space-y-6">
        ...
        <div>
          ...
          <textarea
            ...
            rows={6}
            maxLength={500}
            placeholder="Podelite svoje iskustvo sa organizacijom..."
          />
