...
import type { EventUpdate, EventPublic, EventCategory } from "../../types/api";
...
export function EditEvent() {
  ...
  const [formData, setFormData] = useState<Partial<EventUpdate>>({});
  ...
  const loadEvent = async () => {
    try {
      ...
      const eventData = await publicApi.getEventByTitle(decodedTitle);
      setEvent(eventData);
      
      // Proveri da li je događaj prošao
      const now = new Date();
      const eventEndDate = new Date(eventData.end_date);
      if (eventEndDate < now) {
        setError("Ne možete izmeniti događaj koji se već desio. Možete samo pregledati detalje.");
      }
      
      // Parse dates into separate date and time
      ...
      const start = parseDateTime(eventData.start_date);
      const end = parseDateTime(eventData.end_date);
      ...
      setFormData({
        ...
        max_volunteers: eventData.max_volunteers || null,
      ...
      setError("Događaj nije pronađen");
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Get event ID from various possible fields
    const eventId = (event as any).id || (event as any)._id;
    if (!eventId) {
      console.error("Event missing ID:", event);
      setError("ID događaja nije pronađen. Pokušajte ponovo.");
      return;
    }
    ...
    if (eventEndDate < now) {
      setError("Ne možete izmeniti događaj koji se već desio.");
      return;
    }
    ...
    try {
      ...
      showToast("Događaj je uspešno ažuriran!", "success");
      setTimeout(() => {
        navigate("/org/dashboard");
      }, 1500);
    } catch (err: any) {
      const errorMessage = err?.message || err?.detail || "Neuspešno ažuriranje događaja";
  ...
  if (!event) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-[#121212] text-lg font-medium">{error || "Učitavanje..."}</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in-up">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-4xl font-bold text-[#121212]">Izmeni događaj</h1>
      </div>

      {error && (
        <div className={`card p-4 mb-6 ${
          isPast 
            ? "bg-cream/50 border-2 border-cream" 
            : "bg-red-50 border-2 border-red-300"
        }`}>
          <p className={`font-medium ${isPast ? "text-[#121212]" : "text-red-700"}`}>
            {error}
          </p>
        </div>
      )}
      ...
      {isPast && (
        <div className="card p-4 mb-6 bg-cream/50 border-2 border-cream">
          <p className="text-[#121212] font-medium">
            ⚠️ Ovaj događaj se već desio. Ne možete izmeniti prošle događaje. Možete samo pregledati detalje.
          </p>
        </div>
      )}
